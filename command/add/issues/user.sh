...
USAGE
  gh project-manager add issues user <project-number> <path-to-data> [flags]

FLAGS
  --help, -h        Show help for command

EXAMPLES
  $ gh project-manager add issues user 98 ./data.json

LEARN MORE
  Use 'gh project-manager add issues user --help' for more information about a command.
...
PROJECT_ID=$(echo "$QUERIED_PROJECT" | jq ".data.user.projectNext.id")
FIELD_ID=$(echo "$QUERIED_PROJECT" | jq ".data.user.projectNext.fields.nodes[] | select(.name == \"Status\") | .id")
OPTIONS=$(echo "$QUERIED_PROJECT" | jq ".data.user.projectNext.fields.nodes[] | select(.name == \"Status\") | .settings | fromjson | .options")
